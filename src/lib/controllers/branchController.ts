...
export async function createBranch(
  ...
  try {
    const validatedData = validateBranchData(branchData);
    ...
    return validateBranchData(newBranch);
  } catch (error) {
...
export async function updateBranch(
  ...
  try {
    const validatedData = validateBranchData(updatedData);
    const response = await fetch(`/api/branches/${id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
    ...
    return validateBranchData(updatedBranch);
  } catch (error) {
